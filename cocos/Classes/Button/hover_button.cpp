...
hover_button* hover_button::create(const std::string& plist_name, const std::string& sprite_framename)
{
    ...
    button->default_sprite = cocos2d::SpriteFrameCache::getInstance()->getSpriteFrameByName(sprite_framename + "-right.png");
    button->hover_sprite = cocos2d::SpriteFrameCache::getInstance()->getSpriteFrameByName(sprite_framename + "-right.png");
    button->click_sprite = cocos2d::SpriteFrameCache::getInstance()->getSpriteFrameByName(sprite_framename + "-right-clicked.png");
...
// 设置按钮的功能函数
void hover_button::setButtonFunc(const std::function<void()>& func) {
    button_func = func;  // 将传入的函数保存在成员变量中
}